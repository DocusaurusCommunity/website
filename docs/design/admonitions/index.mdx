...
Inspect is key to figuring out how to override the appearnce of the defualt admonitions.
...
and use the seach bar to find the "note" admonition's class: `alert--secondary`.
...
colors to use. We see four attributes and their corresponding defualt variables:
...
[Diagram that points to what each admoinition variable looks like on an example "note" admonition](admonition-var-diagram.png)

Let't say we want to override the background of the "note" admonition to purple. Well,
...
This is where we can add permanant overrides to the CSS of our site, which means we can update the
...
"note" admonition's colora to an example of colors.
...
CSS class. Updte the static RGB value of `.alert--secondary` to our new variable, using `var()`.
...
Now, lets add a color for when the page is in dark mode. The procedure is very similar to the
